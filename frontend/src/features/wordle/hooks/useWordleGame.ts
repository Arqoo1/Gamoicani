...
import {
  ...
  saveWordleProgress,
  getProgressKey,
} from "@/features/wordle/model/storage";
import {
  GameStatus,
...
import {
  ...
  WORD_LENGTH,
  WordsJson,
} from "@/features/wordle/model/screenModel";
import {
  BACKSPACE_KEY,
  ENTER_KEY,
  GEORGIAN_LETTERS,
...
type GameAction =
  | {
      type: "HYDRATE";
      payload: { guesses: string[]; currentLetters: string[]; gameStatus: GameStatus; message: string };
    }
  ...
  | {
      ...
      payload: { guess: string; isWin: boolean; isLoss: boolean; answer: string; tutorialMessage?: string };
    }
...
export function useWordleGame(
  ...
  const validWords = new Set(
    [...wordData.answers, ...wordData.validWords]
      .map((w) => w.trim())
      .filter((w) => splitWord(w).length === WORD_LENGTH)
  );
  ...
  const answer =
    gameMode === "tutorial"
      ? "სახლი"
      : (answers[(dailyAnswerIndex + state.answerOffset) % answers.length] ?? "სახლი");
    ...
    loadWordleProgress(progressKey)
      ...
      .catch(() => {
        if (active)
          dispatch({
            type: "HYDRATE",
            payload: { guesses: [], currentLetters: [], gameStatus: "playing", message: DEFAULT_MESSAGE },
          });
      });
