...
export async function runAuthBootstrap(user: AuthUser, updateUser: (next: AuthUser) => void): Promise<void> {
  try {
    ...
    if (userPlays === 0) {
      const { getRepairCompletions } = await import("@/features/wordle/model/storage");
      const completions = await getRepairCompletions();

      if (completions.length > 0) {
        const { repairStats } = await import("@/features/scores/api/scoresApi");
